...
wordList=["我在玉龙雪山","我喜欢玉龙雪山","我还要去玉龙雪山"]
def JieBa(line):
    segs=jieba.lcut(line)
    print(segs)
# 切词
textTest=[[word for word in jieba.lcut(words)] for words in wordList]
# 生成字典
...
file_path=os.getcwd()+'\\dict.dict'
dictionary.save(file_path)
dictionary.save_as_text(file_path,  sort_by_word=True)
...
# dictionary.load_from_text(file_path)
# 遍历字典
...
# 过滤字典
# dictionary.filter_extremes(no_below=2, no_above=0.8,keep_n=3 ) #过滤字典,过滤词的出现数量小于2，词频>0.8,且只取前3项 (未实现)
...
# 切词
test=[word for word in jieba.lcut(wordsTest)]
print(test)
...
testword = "我去玉龙雪山"
print([word for word in jieba.lcut(testword)])
test_bow = dictionary.doc2bow([word for word in jieba.lcut(testword)])
